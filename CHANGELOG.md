...
### Added & Improved
- **Autonomous Direct Expense Auto-Commit (Auto-Pilot Stage 3) 🚀**: Unambiguous incoming transactions with certified high-confidence categorization ($\ge 85\%$) are now automatically recorded directly to accounts with reconciliation dates, audit trail logging, and complete rollback capability when Auto-Pilot is active. Multi-category and provisional merchants strictly remain in the review cockpit for human review.
...
- **Smart Label Test Sandbox & Live Simulation 🧪**: Added an interactive test sandbox directly in settings allowing real-time testing of the entire smart label decision cascade (manual/learned rules $\to$ historical matching $\to$ chameleon merchants $\to$ local Ollama AI fallback). Features smooth transition animations, stable layout sizing, instant previous result clearing, and 1-click permanent rule creation.
- **AI Habit Detective & Naming with Anti-Garbage Guardrails 🤖**: Enhanced smart label resolution to optionally query local Ollama LLMs with the user's historical naming catalogue and custom categories when deterministic matching fails. Enforced strict anti-garbage validation requiring token overlap with the raw label or catalogued merchant patterns to discard low-quality hallucinations.
- **Smart Label Reliability & Manual Rule Protection 🏷️**: User-configured label rules are now strictly protected from being overwritten by automated imports. Added progressive learning ($N \ge 2$) to prevent premature category locking, statistical category dispersion detection, and native multi-category handling for general merchants (Amazon, PayPal, supermarkets) to eliminate wrong suggestions.
- **Reversible Smart Label Management & Global Undo Integration ↩️**: Made rule protection (manual vs auto) and multi-category mode fully reversible with 1-click toggles, interactive table chips, and an in-place edit modal with instant accent- and case-permissive category search, live selection preview, and keyboard navigation. Integrated rule modifications into the application's global ActionHistory audit trail and added instant toast undo actions.
- **Review Cockpit Smart Suggestion Transparency Badges 💡**: Added contextual provenance badges (`🛡️ Règle manuelle`, `🤖 Règle apprise`, `⚠️ Provisoire (1ère fois)`, `🔀 Multi-catégories`, `🕒 Historique`) with rich multi-line tooltips directly in the Sas d'attente (Sync & Import review modal). Users now immediately see how descriptions and categories were deduced and exactly how/where to adjust them if mismatched.
...
- **Bank Sync Trigger Provenance & Notification Deduplication 🔔**: Added trigger context to bank synchronization notifications (`vault_unlock`, `scheduled`, `manual`) so users clearly see what initiated each sync attempt. Implemented smart in-place notification deduplication to prevent stacking repetitive error cards for the same bank connection.
- **Strict 3-Hour Cooldown & Test Isolation ⏱️**: Enforced the persistent 3-hour anti-spam cooldown across scheduler loop cycles and background sync triggers, preventing rapid repeated attempts. Isolated automated test suites from executing background threads against real bank connections.
- **Batch Ollama AI Categorization Fallback 🤖**: Implemented Stage 3 batch prompting (`call_ollama_batch`) to classify unknown merchants in a single consolidated JSON request (2-3s total latency), with strict anti-hallucination guards ensuring only user-defined categories are assigned, and offline failsafes guaranteeing zero exceptions or disruptions when Ollama is unavailable.
