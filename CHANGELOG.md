...
### Added & Improved
- **Forced Manual Bank Sync & 2FA Auto-Prompt 📱**: Manual clicks on 'Relevé en ligne' now immediately force online synchronization (bypassing the 3-hour automated background cooldown), automatically surface the interactive 2FA modal when strong customer authentication is required (e.g. Crédit Mutuel, Crédit Agricole), and auto-close upon mobile approval. Added persistent Woob storage to preserve 90-day SCA session tokens.
- **Desktop Startup Resilience & Double Packaging Smoke Tests 🛡️**: Introduced automated dual-stage smoke tests (sidecar `/api/health` validation + post-build administrative MSI extraction test) to guarantee 100% functional release bundles, added a startup failure recovery screen with direct diagnostic logs and fallback release download link, and secured sidecar runtime assets in versioned packaging specs.

## [1.1.6] - 2026-09-09

### Added & Improved
- **100% Offline Zero-Cloud Privacy 🔒**: Fully decoupled all external CDN assets (Chart.js, KaTeX, Marked.js, DOMPurify, fonts, flags) into local offline bundles. OmniBank now operates entirely offline with zero third-party network requests.
...
### Fixed
- **Bank Sync Failure Alerting 🔔**: Bank sync errors now trigger a prominent red toast notification with a 1-click shortcut to the notification drawer, and sync buttons immediately reflect error status instead of prematurely indicating success.
- **Bank Outage & Maintenance Differentiation 🏦**: Improved connection diagnostics to clearly distinguish between temporary bank-side maintenance/outages and actions requiring user intervention.
- **Interactive AI Chat Corrections 💬**: Fixed raw JSON blocks leaking into chat responses, enabling smooth 1-click interactive approval for transaction adjustments suggested by the AI assistant.
- **Cash Flow Simulator Accuracy 📈**: Corrected salary revenue filtering so recurring paychecks and seasonal bonuses are reliably factored into long-term cash flow projections.
- **Merchant Name & P2P Transfer Cleanup 👤**: Prevented payment provider names (Wero, Paylib, Lydia) from overwriting actual sender/recipient identities or wiping custom notes.
- **Multi-Page PDF Reports 📄**: Resolved PDF export truncation to ensure complete, multi-page financial statements with clean page breaks and no overlapping UI elements.
- **Analytics View Filters & Toolbar 📊**: Fixed table headers overlapping the filter bar in the Financial Analytics view, and automatically hid the inactive period dropdown when custom dates are selected.
- **Sidebar Loan Carousel 🏦**: Hid carousel arrows when no loan accounts are configured, preventing confusion with checking accounts.
- **Diagnostic Version Accuracy 🏷️**: Fixed system diagnostics to report the accurate running application version in desktop builds.
