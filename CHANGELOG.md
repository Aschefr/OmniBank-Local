...
### Added & Improved
- **Modularized Incremental SQLite Migration Engine 🗄️**: Deconstructed the monolithic 703-line `app/init_data.py` into a robust modular migration architecture under `app/migrations/`. Encapsulated 24 numbered, sequential version modules (`v02` to `v25`) under `app/migrations/versions/` orchestrated by a transactional runner with structured French logging, safe idempotent column addition helpers (`safe_add_column`), fast-path target version bypass (<1ms on startup/profile switches), and a comprehensive test suite (`test_migrations_integrity.py`) boosting automated test coverage to 263 tests with 100% pass rate.
- **Modularized Design System & Stylesheet Architecture 🎨**: Refactored the monolithic 13,919-line `style.css` into 21 structured domain stylesheets across 5 semantic directories (`static/css/base/`, `static/css/themes/`, `static/css/components/`, `static/css/views/`, `static/css/responsive/`) orchestrated by a master `@import` manifest. Maintained 100% cascade rule order, selector fidelity, zero CSS bundle build dependencies, and added a dedicated non-regression test suite (`test_css_modular_integrity.py`) bringing automated test coverage to 257 tests.
- **Modularized Core Frontend Application Architecture 🏗️**: Modularized the monolithic 3,149-line `app.js` into a lightweight lifecycle orchestrator (~460 lines) and five focused domain sub-modules under `static/js/app/` (`notifications.js`, `sidebar.js`, `profiles.js`, `changelog.js`, `i18n_picker.js`) seamlessly mounted onto `App.prototype`, preserving 100% inline event handler compatibility and zero regression across the entire automated test suite.
...
- **Smart Match Substring Collision on Bank Labels 🎯**: Prevented small noise-cleaned substrings (such as `"CA"` in `"CA Remboursement Cotisation carte"`) from erroneously matching unrelated operations through token-boundary matching (`"VIR INST WERO de YOANN VILACA"` was matching on `"VILACA"`). Added native WERO instant transfer pattern recognition for clean income fallback.
- **P2P Transfer Name Truncation & Staging Overwrites 👤**: Prevented standalone payment app tokens (`"Wero"`, `"Paylib"`, `"Lydia"`) from wiping out sender/recipient identities during AI categorization. Protected customized transaction descriptions from being destructively overwritten during live staging re-evaluation.
- **AI Staging Badge & Category Persistence Across Reopen 💾**: Fixed an issue where closing and reopening the review modal demoted AI-suggested categories and badges (`🤖 AI Suggestion (85%)`) back to fallback (`🛡️ Fourre-tout` / `Revenus divers`) due to proactive re-resolution overriding existing AI enrichments. Implemented client-server synchronization via `/api/bank-sync/update-pending` and anti-demotion guards.
- **Contextual Smart Label & AI Origin Indicators 💡✨**: Replaced the static generic lightbulb tooltip (`"Suggested from your history"`) across pending dashboard operations and unreconciled views with dynamic origin icons and comprehensive tooltips (`✨ Local AI Suggestion (85%)`, `🤖 Learned Rule`, `🛡️ Manual Rule`, `🕒 History`, `🔀 Multi-Category`, `🛡️ Fallback`), accurately reflecting transaction intelligence source.
- **PDF Export Multi-Page Pagination & Page Breaks 📄**: Fixed document truncation to a single page in PDF print previews by removing fixed-height and hidden overflow constraints on root elements during print, switching sticky headers and totals to static layout for print output, and properly styling inserted manual page breaks (`.print-page-break-before-forced`). Resolved on-screen flickering and isolated background printing by dismissing active toast notifications prior to printing and strictly hiding all non-report elements (`.app-toast`, modals, app body) in `@media print`.
- **Analytics Sticky Toolbar Layering 📊**: Fixed scrolling table headers and total rows overlapping the top filter bar in the Financial Analytics (`Synthèse`) view by adjusting the toolbar's stacking context (`z-index: 500`).
- **Analytics Custom Range Period Selector Dynamic Display 📅**: In the Financial Analytics view, toggling custom date range mode now immediately conceals the inactive period selector dropdown from the toolbar instead of leaving it visible.
- **Sidebar Loan Accounts Carousel Display 🏦**: Corrected inverted filtering logic in `sidebar.js` where liquid accounts were erroneously recognized as loan accounts, which caused the carousel navigation (`◀ 1/2 ▶`) to display and mirror liquid accounts under "Prêts & Emprunts" even when no loans existed. Carousel controls are now hidden and state defaults cleanly to liquid accounts.

## [1.1.5] - 2026-09-09

### Added & Improved
- **Autonomous Direct Expense Auto-Commit (Auto-Pilot Stage 3) 🚀**: Unambiguous incoming transactions with certified high-confidence categorization are now automatically recorded directly to accounts with reconciliation dates, audit trail logging, and complete rollback capability when Auto-Pilot is active. Multi-category and provisional merchants strictly remain in the review cockpit for human review.
- **AI-Assisted Review & Live Progress Feedback 🧠**: The review cockpit now automatically queries local Ollama AI in the background to suggest smart categories and clean merchant names for uncategorized transactions, with distinct provenance badges (`🛡️ Manual Rule`, `🤖 Learned Rule`, `🕒 History`, `🔀 Multi-category`, `🤖 AI Suggestion`) and a non-blocking animated status banner.
- **Frictionless Statement Import Dropzone 📥**: Statement imports where 100% of transactions are autonomously handled by Auto-Pilot now automatically close the upload dialog and display a celebratory toast summary, refreshing account balances and eliminating empty review dialogs.
- **Smart Label Simulation Sandbox & Reversible Rules 🧪**: Added an interactive test sandbox directly in settings allowing real-time simulation of merchant recognition, rule matching, and local AI fallback. Manual rule protection and multi-category mode are fully reversible with 1-click toggles and instant undo support.
- **Bank 2FA Requalification & Background Sync Management 📱**: Bank connections requiring two-factor authentication now display contextual `"Validate on smartphone"` actions with dedicated notifications. Application startup is fully decoupled from background AI reporting for instant boot.
